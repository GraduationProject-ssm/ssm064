...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=62)
...
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此农产品仓库管理系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
|3|order\_name|varchar(200)|订单名   |是|
|4|order\_types|int(11)|委托类型   |是|
|5|insert\_time|timestamp|委托时间   |是|
|6|caozuo\_name|varchar(200)|操作人姓名   |是|
|7|caozuo\_table|varchar(200)|操作人所在表名|是|
|8|caozuo\_types|int(11)|操作类型   |是|
|9|update\_time|timestamp|操作时间|是|
|10|create\_time|timestamp|创建时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|id|int(11)|主键|否|
|2|entrust\_in\_out\_order\_id|int(11)|订单id|是|
|3|goods\_id|int(11)|物资表id|是|
|4|order\_number|int(11)|数量   |是|
|5|entrust\_types|int(11)|状态   |是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|id|int(11)|主键|否|
|2|goods\_name|varchar(200)|物品名字  |是|
|3|goods\_types|int(11)|物品种类   |是|
|4|goods\_number|int(11)|物资数量   |是|
|5|goods\_photo|varchar(200)|物品图片|是|
|6|danwei|varchar(200)|单位|是|
|7|goods\_content|varchar(200)|物资详情|是|
|8|flag|int(11)|是否删除|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|id|int(11)|主键|否|
|2|order\_name|varchar(200)|订单名   |是|
|3|caozuo\_name|varchar(200)|操作人姓名   |是|
|4|caozuo\_table|varchar(200)|操作人所在表名|是|
|5|order\_types|int(11)|类型   |是|
|6|insert\_time|timestamp|出入库时间   |是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|id|int(11)|主键|否|
|2|in\_out\_order\_id|int(11)|订单id|是|
|3|goods\_id|int(11)|物资表id|是|
|4|order\_number|int(11)|数量   |是|
|5|create\_time|timestamp|创建时间|是|
